...
delay :: CompilingDefault uni fun m => PIRTerm uni fun -> m (PIRTerm uni fun)
delay body = PIR.LamAbs () <$> liftQuote (freshName "thunk") <*> compileType GHC.unitTy <*> pure body

delayType :: CompilingDefault uni fun m => PIRType uni -> m (PIRType uni)
delayType orig = PIR.TyFun () <$> compileType GHC.unitTy <*> pure orig

delayVar :: CompilingDefault uni fun m => PIRVar uni fun -> m (PIRVar uni fun)
...
maybeDelay :: CompilingDefault uni fun m => Bool -> PIRTerm uni fun -> m (PIRTerm uni fun)
maybeDelay yes t = if yes then delay t else pure t

maybeDelayVar :: CompilingDefault uni fun m => Bool -> PIRVar uni fun -> m (PIRVar uni fun)
maybeDelayVar yes v = if yes then delayVar v else pure v

maybeDelayType :: CompilingDefault uni fun m => Bool -> PIRType uni -> m (PIRType uni)
