...
fvTerm = cata f
  where
    ...
    f (UnwrapF _ t)     = t
    f (IWrapF _ _ _ t)  = t
    f (ConstrF _ _ _ t) = unions t
    f (CaseF _ _ t cs)  = t `union` unions cs
...
ftvTerm = cata f
  where
    ...
    f (ConstrF _ ty _ t)   = ftvTy ty `union` unions t
    f (CaseF _ ty t cs)    = ftvTy ty `union` t `union` unions cs
    f VarF{}               = Set.empty
    f ConstantF{}          = Set.empty
    f BuiltinF{}           = Set.empty
