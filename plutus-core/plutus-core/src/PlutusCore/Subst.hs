...
termSubstFreeNamesA f = go Set.empty where
    ...
    go bvs (Prod ann args)            = Prod ann <$> traverse (go bvs) args
    go bvs (Proj ann i term)          = Proj ann i <$> go bvs term
    go bvs (Tag ann ty i p)           = Tag ann ty i <$> go bvs p
...
fvTerm = cata f
  where
    ...
    f (UnwrapF _ t)     = t
    f (IWrapF _ _ _ t)  = t
    f (ProdF _ t)       = unions t
    f (ProjF _ _ t)     = t
    f (TagF _ _ _ t)    = t
    f (CaseF _ t cs)    = t `union` unions cs
...
ftvTerm = cata f
  where
    ...
    f (ProdF _ t)          = unions t
    f (ProjF _ _ t)        = t
    f (TagF _ ty _ t)      = ftvTy ty `union` t
    f (CaseF _ t cs)       = t `union` unions cs
    f VarF{}               = Set.empty
    f ConstantF{}          = Set.empty
    f BuiltinF{}           = Set.empty
...
ftvTy = cata f
  where
    ...
    f (TyProdF _ tys)        = unions tys
    f (TySumF _ tys)         = unions tys
